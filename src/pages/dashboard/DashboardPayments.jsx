...
const payments = [
  { id: 'PAY001', bookingId: 'BK001', amount: 22500, status: 'success', date: '2026-06-28', method: 'UPI' },
  { id: 'PAY002', bookingId: 'BK002', amount: 10000, status: 'success', date: '2026-06-20', method: 'Card' },
  { id: 'PAY003', bookingId: 'BK003', amount: 10500, status: 'pending', date: '2026-06-15', method: 'Net Banking' },
];
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              {['Payment ID', 'Booking', 'Amount', 'Method', 'Date', 'Status'].map((h) => (
          ...
          <tbody className="divide-y">
            {payments.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4"><Badge variant={p.status === 'success' ? 'success' : 'warning'}>{p.status}</Badge></td>
              </tr>
            ))}
          </tbody>
        </table>
