...
const BookingSuccess = () => {
  ...
  const { booking, car, startDate, endDate, pricing } = location.state || {};
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-16 text-center">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mt-6">Booking Request Submitted!</h1>
      <p className="text-gray-500 mt-2">The host will review your request and respond shortly.</p>

      {booking && (
        <div className="flex justify-center mt-4">
          <Badge variant={BOOKING_STATUS_VARIANT[booking.status] || 'warning'}>
            {booking.status}
          </Badge>
        </div>
      )}
        ...
        <div className="bg-gray-50 rounded-xl p-6 mt-8 text-left space-y-3">
          ...
          <div className="flex justify-between text-sm pt-3 border-t">
            ...
            <span className="font-semibold text-primary-600">
              {formatCurrency(booking?.totalAmount ?? pricing?.total ?? 0)}
            </span>
          </div>
        </div>
      )}
